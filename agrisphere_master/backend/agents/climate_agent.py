...
from backend.agents.disaster_monitoring_agent import DisasterMonitoringAgent
from backend.agents.farmer_alert_agent import FarmerAlertAgent
    ...
    def run_tier1_pipeline(self, region: str) -> Dict[str, Any]:
        ...
        if self.model:
            try:
             ...
             print("⚠️ [Agentic AI Fallback] GEMINI_API_KEY not found. Falling back to deterministic logic.")
             use_fallback = True
        ...
        return {
            "agent": "climate_agent", # Unified standard requires this top-level field for orchestrator
...
def simulate_climate_demo() -> Dict[str, Any]:
    ...
    with patch('backend.agents.disaster_monitoring_agent.get_weather', return_value=severe_mock_data):
